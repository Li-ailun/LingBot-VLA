...
class LingbotVLAServer:
    ...
    def load_vla(self, path_to_pi_model) -> LingbotVlaPolicy:
        ...
        # Force config switches required by robbyant/lingbot-vla-4b base checkpoint.
        # The checkpoint contains qwen_expert layernorm beta/gamma weights,
        # so AdaNorm replacement must be enabled before building the policy.
        config.adanorm_time = True
        config.split_gate_liner = True
        config.no_split_gate_liner = False
        config.final_norm_adanorm = False
        config.norm_qkv = False

        # Inference-time defaults.
        config.train_state_proj = False
        config.train_expert_only = False
        config.resize_imgs_with_padding = False

        print("[deploy patch] adanorm_time =", config.adanorm_time)
        print("[deploy patch] split_gate_liner =", config.split_gate_liner)
        print("[deploy patch] no_split_gate_liner =", config.no_split_gate_liner)
        print("[deploy patch] final_norm_adanorm =", config.final_norm_adanorm)
        print("[deploy patch] norm_qkv =", config.norm_qkv)

        policy = LingBotVlaInferencePolicy(config, tokenizer_path=base_model_path)

        # Debug: check whether AdaNorm beta/gamma modules exist in constructed model.
        beta_gamma_keys = [
            name for name, _ in policy.named_parameters()
            if (
                "qwen_expert" in name
                and (
                    ".input_layernorm.beta." in name
                    or ".input_layernorm.gamma." in name
                    or ".post_attention_layernorm.beta." in name
                    or ".post_attention_layernorm.gamma." in name
                )
            )
        ]
        print("[debug] beta/gamma params in constructed policy:", len(beta_gamma_keys))
        for k in beta_gamma_keys[:20]:
            print("[debug]", k)
    ...
    def resize_image(self, observation):
        ...
        print("[debug resize_image] expected image_features:", image_features, flush=True)
        print("[debug resize_image] received observation keys:", sorted(list(observation.keys())), flush=True)
        for image_feature in image_features:
            if image_feature not in observation:
                raise AssertionError(
                    f"missing image key: {image_feature}; received keys: {sorted(list(observation.keys()))}"
                )
